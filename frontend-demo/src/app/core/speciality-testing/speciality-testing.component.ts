...
@Component({
  selector: 'app-speciality-testing',
  templateUrl: './speciality-testing.component.html',
  styleUrls: ['./speciality-testing.component.css'],
})
export class SpecialityTestingComponent implements OnInit {
  ...
  constructor(private toastr: ToastrService,
    public tableChange: ChangeDetectorRef, private navbarService: NavbarService, private specialityService : SpecialityService,
    private cartService: CartService, private cookieService: CookieService, private sharedservice: SharedService, private serviceMethod: PostmethodService, private encryptDecrypt: EncryptdecryptService, private router: Router, private modalService: BsModalService, private cdr: ChangeDetectorRef, private landingHomeService: LandingHomeService,
   ) {
    this.specialityName = history.state.name;
  ...
  filteredSpecialityList: any[] = [];
  async getSpeciality() {
    try {
      const getSpeciality = await this.specialityService.getSpecialityDetails();
      ...
      if (this.encrydecryresponse.data.statusCode == 200) {
        ...
        if (this.isActiveTab == 1) {
          this.specialityTestingList = this.encrydecryresponse.data.responseObj.filter((val: any) => val.isGender == 1)
           this.specialityTestingList =  [...new Set(this.specialityTestingList)]
        } else {
          this.specialityTestingList = this.encrydecryresponse.data.responseObj.filter((val: any) => val.isGender == 2)
          this.specialityTestingList =  [...new Set(this.specialityTestingList)]
        }
          // Remove duplicates by sampleType name (assuming sampleType is the name you want to use to filter)
          const uniqueSampleTypes = this.specialityTestingList.filter((value: { SpecialityName: any; }, index: any, self: any[]) =>
            index === self.findIndex((t: { SpecialityName: any; }) => (
                t.SpecialityName === value.SpecialityName
            ))
        );

        this.specialityTestingList = [...uniqueSampleTypes];
        
        // this.filteredSuggestions = this.encrydecryresponse.data.responseObj
        console.log("getSpeciality: ", this.specialityTestingList);
        this.filteredSpecialityList = this.specialityTestingList;
        ...
        .subscribe(filtered => {
          this.filteredSpecialityList = filtered;
  ...
  trackByFn(index: number, item: any) {
    return item.SpecialPkId;
  }

  private _filter(value: string): any[] {
    const filterValue = value.toLowerCase();
    return this.specialityTestingList.filter((speciality: any) =>
      speciality.SpecialityName.toLowerCase().includes(filterValue)
    );
    

  }

  getSpecialityName(specialityName:any){
    console.log('Speciality clicked:', specialityName);
  ...
  navigateToOverview(data: any) {
    this.router.navigate(["/overview"], { state: { testPk: data.TESTPK, orgId: data.orgId, location: this.selectedLocation, isAdded: data.isAdded, fromPage: "speciality-testing", name: this.specialityName, item: data,tabName:'Speciality Segment' } })
  }
...
item.timeOfAddition = `${day} ${month} ${year} ${hours}:${minutes}:${seconds}`;
  ...
  if(sessionStorage.getItem('location') || localStorage.getItem('location')){
    ...
    this.elasticSearchRequest.specilityName = this.specialityName !='0' ? this.specialityName : ''
  ...
  async getViewScheduleAndTAT(data: any): Promise<void> {
    ...
    console.log('speciality viewScheduleAndTAT req:', req);
    this.requestModel = new RequestModel();
    this.encrydecryresponse = new EncryptDecryptResponse();
    this.encrydecryresponse = this.encryptDecrypt.encryption(req);
    this.requestModel.payload = this.encrydecryresponse.data;
    await this.landingHomeService.getViewScheduleAndTAT(this.requestModel, this.encrydecryresponse.iv).toPromise().then((response: any) => {
      ...
      this.responseModel = response.body;
      this.encrydecryresponse = this.encryptDecrypt.decryption(this.responseModel.responseData, clientSecret);
      console.log('speciality viewScheduleAndTAT res:', this.encrydecryresponse.data);
