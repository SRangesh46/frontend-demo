...
import { TestAlgorithmService } from '../test-algorithms/test-algorithm.service';
@Component({
...
export class ListTestComponent implements OnInit {
  ...
  constructor(private toastr: ToastrService, private cartService: CartService, 
    private cookieService: CookieService, public tableChange: ChangeDetectorRef, private navbarService: NavbarService, private sharedservice: SharedService, private encryptDecrypt: EncryptdecryptService, private router: Router, private modalService: BsModalService, private cdr: ChangeDetectorRef, private landingHomeService: LandingHomeService, private testService: TestAlgorithmService,private listTestService: ListTestService) {
    ...
    console.log(history.state.name, 'name');
  ...
  //   this.createForm()
  //   this.getCartItems();
  //   this.dataSubscription=this.navbarService.currentData.subscribe((res: any) => {
  //     console.log(res, 'res');
  //    this.getCurrentPath();
  //     if (res == "location" && this.currentPath == '/list' || res=='location') {
  //       //debugger
  ...
  //   }
  //   this.dataSubscription1=this.navbarService.currentElasticRequestData.subscribe((res: any) => {
  //     this.elasticSearchRequest = res
  //     console.log(res)
  //     this.searchedWords = this.elasticSearchRequest.keyword ? this.elasticSearchRequest.keyword : this.elasticSearchRequest.globalSearch
  //     this.testForm.patchValue({
  //       department: this.elasticSearchRequest.department,
  //       category: this.elasticSearchRequest.categoryName,
  //       method: this.elasticSearchRequest.methodName,
  //       reportOn: this.elasticSearchRequest.reportedOn,
  //       status: this.elasticSearchRequest.isAvtice,
  //       disease: this.elasticSearchRequest.diseasesName,
  //       speciality: this.elasticSearchRequest.specilityName
  //     })
  //     //debugger
  //     this.getTestData()
  //   })
  ...
  //         this.methodMasterList =  [...new Set(this.methodMasterList)]
  //       }
  //       // console.log("getMethodMaster: ", this.methodMasterList);
  //     } else {
  //       // this.isLoading = new BehaviorSubject<boolean>(false);
  //     }
  //   });
  ...
  //     let clientSecret = response.headers.get('clientsecret')
  //     this.responseModel = response.body
  //     console.log(this.responseModel, 'responsemodal');
  ...
  //       }, 100)
  //       this.getTotalPrice()
  //       console.log("item: ", item.TESTPK);
  //       console.log(JSON.stringify(this.addedCartItems));
  ...
  //       localStorage.setItem('cartItems', JSON.stringify(this.cartService.addedCartItem));
  //       this.cartService.changeData((this.cartService.addedCartItem).toString())
  //       this.getTotalPrice()
  //       this.insertUpdateTestCartDetails();
  ...
  navigateToOverview(data: any) {
    this.router.navigate(["/overview"], { state: { testPk: data.TESTPK, orgId: data.orgId, location: this.selectedLocation, isAdded: data.isAdded, fromPage: "new-test", name:"New Tests",item: data } })
  }
...
public addedCartItems: any = []   // ← ADD this line
attuneFlag: boolean = localStorage.getItem('attune')?.toUpperCase() === 'Y';
selectedScheduleTest: any = null;
// isLoading already declared at line 38 — do NOT redeclare here
public elasticSearchRequest: any = {}
clusterName:any;
orgId:any;
  ...
  if(sessionStorage.getItem('location') || localStorage.getItem('location')){
    ...
    this.elasticSearchRequest.testType = "INV"
  ...
  async getViewScheduleAndTAT(data: any) :Promise<void> {
    ...
    console.log('newTest viewScheduleAndTAT req:', req);
    this.requestModel = new RequestModel();
    this.encrydecryresponse = new EncryptDecryptResponse();
    this.encrydecryresponse = this.encryptDecrypt.encryption(req);
    this.requestModel.payload = this.encrydecryresponse.data;
   await this.listTestService.getViewScheduleAndTAT(this.requestModel, this.encrydecryresponse.iv).toPromise().then((response: any) => {
      this.responseModel = new ResponseModel();
      const clientSecret = response.headers.get('clientsecret');
      this.responseModel = response.body;
      this.encrydecryresponse = this.encryptDecrypt.decryption(this.responseModel.responseData, clientSecret);
      console.log('newTest viewScheduleAndTAT res:', this.encrydecryresponse.data);
