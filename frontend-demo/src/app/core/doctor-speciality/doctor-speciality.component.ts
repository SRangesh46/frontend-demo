...
import { SpecialityService } from '../speciality-testing/speciality.service';
...
export class DoctorSpecialityComponent implements OnInit {
  ...
  clearDoctorspecialityName(){
    this.doctorspecialityName=''
    this.getTestData();
  ...
  constructor(private toastr: ToastrService, public tableChange: ChangeDetectorRef, private navbarService: NavbarService, private cartService: CartService, private cookieService: CookieService, private sharedservice: SharedService, private serviceMethod: PostmethodService, private encryptDecrypt: EncryptdecryptService, private router: Router, private modalService: BsModalService, private cdr: ChangeDetectorRef, private landingHomeService: LandingHomeService,
    ...
    this.doctorspecialityName = history.state.name;
    console.log(this.doctorspecialityName);
  ...
  getSpecialityName(specialityName: any) {
    this.doctorspecialityName = specialityName;
    this.getTestData();
  ...
  filteredDoctorSpecialityList: any[] = [];
  async getDoctorSpeciality() {
    try {
      const getSpeciality = await this.specialityService.getDoctorSpecialityDetails();
      ...
      if (this.encrydecryresponse.data.statusCode == 200) {
        if (this.isActiveTab == 1) {
          this.doctorspecialityList = this.encrydecryresponse.data.responseObj.filter((val: any) => val.isGender == 1)
           this.doctorspecialityList =  [...new Set(this.doctorspecialityList)]
        } else {
          this.doctorspecialityList = this.encrydecryresponse.data.responseObj.filter((val: any) => val.isGender == 2)
          this.doctorspecialityList =  [...new Set(this.doctorspecialityList)]
        }
        // Remove duplicates by sampleType name (assuming sampleType is the name you want to use to filter)
        const uniqueSampleTypes = this.doctorspecialityList.filter((value: { doctorSpecialityName: any; }, index: any, self: any[]) =>
          index === self.findIndex((t: { doctorSpecialityName: any; }) => (
              t.doctorSpecialityName === value.doctorSpecialityName
          ))
      );

      this.doctorspecialityList = [...uniqueSampleTypes]; 
        console.log("getDoctorSpeciality: ", this.doctorspecialityList);
        this.filteredDoctorSpecialityList = this.doctorspecialityList
        this.myControl.valueChanges
          ...
          .subscribe(filtered => {
            this.filteredDoctorSpecialityList = filtered;
          });
      }
    } catch (error: any) {
      console.log(error);
  ...
  private _filter(value: string): any[] {
    ...
    return this.doctorspecialityList.filter((speciality: any) =>
      speciality.doctorSpecialityName.toLowerCase().includes(filterValue)
    );
  }
  ...
  navigateToOverview(data: any) {
    this.router.navigate(["/overview"], { state: { testPk: data.TESTPK, orgId: data.orgId, location: this.selectedLocation, isAdded: data.isAdded, fromPage: "doctor-speciality", name: this.doctorspecialityName, item: data ,tabName: 'Doctor Speciality'} })
  }
...
item.timeOfAddition = `${day} ${month} ${year} ${hours}:${minutes}:${seconds}`;
  ...
  getTestData() {
    ...
    this.elasticSearchRequest.specilityName = '',
      this.elasticSearchRequest.doctorSpecialityName = this.doctorspecialityName !='0' ? this.doctorspecialityName : '',
  ...
  async getViewScheduleAndTAT(data: any): Promise<void> {
    ...
    console.log('doctor-speciality viewScheduleAndTAT req:', req);
    this.requestModel = new RequestModel();
    this.encrydecryresponse = new EncryptDecryptResponse();
    this.encrydecryresponse = this.encryptDecrypt.encryption(req);
    this.requestModel.payload = this.encrydecryresponse.data;
    await this.landingHomeService.getViewScheduleAndTAT(this.requestModel, this.encrydecryresponse.iv).toPromise().then((response: any) => {
      ...
      this.responseModel = response.body;
      this.encrydecryresponse = this.encryptDecrypt.decryption(this.responseModel.responseData, clientSecret);
      console.log('doctor-speciality viewScheduleAndTAT res:', this.encrydecryresponse.data);
