...
export class UpdatePackageComponent implements OnInit {
  ...
  private elasticSubscription: Subscription;
  ...
  navigateToOverview(data: any) {
    this.router.navigate(["/overview"], { state: { testPk: data.TESTPK, orgId: data.orgId, location: this.selectedLocation, isAdded: data.isAdded, fromPage: "updated-package", name: "Updated Package", item: data } })
  }
...
item.quantity = 1
item.total = item.testFees;
item.orgId = localStorage.getItem('orgId') || sessionStorage.getItem('orgId'); // ← ADD இந்த line
item.locationId = this.cookieService.get('locationId');
const now = new Date();
const day = String(now.getDate()).padStart(2, '0');
const month = now.toLocaleString('en-GB', { month: 'short' });
...
item.reportTAT = item.ReportTAT || '';
  ...
  if(sessionStorage.getItem('location') || localStorage.getItem('location')){
    ...
    this.elasticSearchRequest.department = this.elasticSearchRequest.department = this.testForm.value.department ? this.testForm.value.department : ''
    ...
    this.elasticSearchRequest.updated = "2"
    ...
    console.log("updated pkg req: ", this.elasticSearchRequest);
    this.requestModel = new RequestModel()
    this.encrydecryresponse = new EncryptDecryptResponse()
    this.encrydecryresponse = this.encryptDecrypt.encryption(this.elasticSearchRequest)
    this.requestModel.payload = this.encrydecryresponse.data
    this.elasticSubscription = this.landingHomeService.elasticgetTests(this.requestModel, this.encrydecryresponse.iv).subscribe((response: any) => {
      this.isLoading = false;
      this.responseModel = new ResponseModel()
      let clientSecret = response.headers.get('clientsecret')
      this.responseModel = response.body
      this.encrydecryresponse = this.encryptDecrypt.decryption(this.responseModel.responseData, clientSecret);
      console.log("updated pkg res: ", this.encrydecryresponse.data);
      if (this.encrydecryresponse.data.length > 0) {
        ...
        this.testDetails.sort((a: any, b: any) => {
          const dateA: any = new Date(
            a.TestUpdateDate.replace(/(\d{2})-(\d{2})-(\d{4})/, "$2/$1/$3")
          );
          const dateB: any = new Date(
            b.TestUpdateDate.replace(/(\d{2})-(\d{2})-(\d{4})/, "$2/$1/$3")
          );
          return dateB - dateA;
        });
  ...
  async getViewScheduleAndTAT(data: any): Promise<void> {
    ...
    console.log('update-package viewScheduleAndTAT req:', req);
    this.requestModel = new RequestModel();
    this.encrydecryresponse = new EncryptDecryptResponse();
    this.encrydecryresponse = this.encryptDecrypt.encryption(req);
    this.requestModel.payload = this.encrydecryresponse.data;
    await this.landingHomeService.getViewScheduleAndTAT(this.requestModel, this.encrydecryresponse.iv).toPromise().then((response: any) => {
      ...
      this.responseModel = response.body;
      this.encrydecryresponse = this.encryptDecrypt.decryption(this.responseModel.responseData, clientSecret);
      console.log('update-package viewScheduleAndTAT res:', this.encrydecryresponse.data);
